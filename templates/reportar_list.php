...
<body>
                    ...
                    <form id="reportForm">
                        ...
                        <div id="contain_select_especie" class="d-flex mb-3">
                            ...
                            <button id="select_especie_2" class="btn btn-secondary flex-fill" data-bs-toggle="modal"
                                data-bs-target="#imageModal" style="display:none;">Subir Imagen</button>
                        ...
                        <div class="text-block mb-3">
                            <label for="cantidad_especie">Cantidad: (Hasta 999)</label>
                            <input id="cantidad_especie" name="cantidad_especie" type="number" maxlength="3"
                                placeholder="Número de individuos" required>
                        </div>
    ...
    <script>
        document.getElementById('reportBtn').onclick = function () {
            ...
            if (especieSeleccionada.value === '') {
                ...
                errorEspecieSel.innerText = 'Debe seleccionar una especie!';
                return;
            }

            // Verifica que todos los campos requeridos están llenos
            if (!fechaAvistamiento.value || !cantidadEspecie.value || !comentarios.value) {
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="text-center mt-3" id="image_warning" style="display: none; color: red;">Solo puedes
                        seleccionar hasta 4 imágenes.</div>
                    <div class="mt-3" id="image_preview_modal" style="display: none;"></div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                    <button type="button" class="btn btn-primary" id="confirm_images">Confirmar</button>
                </div>
            </div>
        </div>
    </div>
                        ...
                        if (!empty($list_specie)) {
                            foreach ($list_specie as $especie) {
                                ...
                                if (!empty($searchQuery)) {
                                    $nombreComun = preg_replace('/(' . preg_quote($searchQuery, '/') . ')/i', '<span class="highlight">$1</span>', $nombreComun);
                                    $familiaEspecie = preg_replace('/(' . preg_quote($searchQuery, '/') . ')/i', '<span class="highlight">$1</span>', $familiaEspecie);
                                }
                                ?>
                                <div class="double-column">
                                    ...
                                    <div class="cardcontain button_center">
                                        <button type="button" class="btn btn-sm select-species" id="boton_select_especie"
                                            ...
                                            data-species-num="<?php echo $num; ?>">Seleccionar</button>
                                    </div>
                                </div>
                                <?php
                            }
    ...
    <script>
        ...
        `;
                ...
                let colombiaBounds = [
                    [-4.23, -81.73],  // Suroeste (lat, lng)
                    [13.39, -66.85]   // Noreste (lat, lng)
                ];

                // Inicializar el mapa
                const map = L.map('map').setView([4.570868, -74.297333], 11); // Coordenadas iniciales (Colombia)
                ...
                // Agregar rectángulo de los límites de Colombia
                const rectangle = L.rectangle(colombiaBounds, { color: "purple", weight: 2 }).addTo(map);
